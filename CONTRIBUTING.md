...
infra/               Dockerfiles and supporting build infrastructure.
docker-bake.hcl      Root Docker Bake build graph.
...
tests/               Golden files, shared fixtures, and e2e assets.
docs/                Plans, ideation, architecture notes.
```
...
Before declaring a slice done, run the checks that match the touched
surface:
...
pnpm --dir js -r test
pnpm --dir js -r build
docker buildx bake --print               # parses bake-hcl
```
