...
test.describe("edit-flag", () => {
  ...
  test("save round-trip rerenders source and shows publish-snapshot toast", async ({
    ...
    // Redirect lands on the view route with ?published=v{N}. The
    // version monotonically increments every time make seed re-runs,
    // so we match the generic shape rather than a specific number.
