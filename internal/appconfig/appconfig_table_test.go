...
func TestLoadProxy_Variants(t *testing.T) {
	cases := []struct {
		...
		{"defaults", "", "", "", "", ":8081", "http://localhost:8080", "", nil},
		{"slug", "", "", "demo", "", ":8081", "http://localhost:8080", "demo", nil},
		{"base", "", "http://api.example.com", "", "", ":8081", "http://api.example.com", "", nil},
		{"ready-flags", "", "", "", "a,b, c ,,", ":8081", "http://localhost:8080", "", []string{"a", "b", "c"}},
		{"all", ":9081", "http://api", "x", "ready", ":9081", "http://api", "x", []string{"ready"}},
	}
	for _, tc := range cases {
		tc := tc
		t.Run(tc.name, func(t *testing.T) {
			...
			t.Setenv("FALSEFLAG_PROXY_READY_FLAGS", tc.ready)
			cfg, err := LoadProxy()
			if err != nil {
				t.Fatal(err)
			}
			if cfg.Addr != tc.wantAddr || cfg.APIBaseURL != tc.wantBase || cfg.ProjectSlug != tc.wantSlug || !slices.Equal(cfg.ReadyFlags, tc.wantReady) {
				t.Errorf("got %+v", cfg)
			}
		})
	}
