...
| GET    | `/readyz`        | Readiness — 200 once the first snapshot poll succeeds, 503 while starting. Returns 200 immediately if no project is configured (idle ready). |
...
| `FALSEFLAG_PROXY_PROJECT_SLUG` | _(empty)_ | Scopes polling to one project. When empty, the proxy still boots and serves `/healthz`, but `/v1/evaluate` returns 503 — useful for compose smoke tests that just check liveness. |
