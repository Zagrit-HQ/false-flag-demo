...
```bash
brew install go pnpm docker           # or your platform equivalents
pnpm --dir js install
...
The conference-talk script (`docs/demo/script.md`) builds on this.
...
- `infra/{Dockerfile,Dockerfile.dashboard}` and `docker-bake.hcl` —
  container builds. `compose.yaml` lives at the repo root.
- `docs/{plans,ideation,architecture,demo}` — planning, architecture,
  and demo docs.
...
ladder (`go test ./...`, `pnpm -r test`, `make sdk-conformance`,
`make contract-test`) is the source of truth.
...
validation ladder. See [AGENTS.md](./AGENTS.md) before adding new
top-level areas.
