...
This is the "before Depot" picture of FalseFlag's CI. The entrypoint
workflow — `.github/workflows/ci.yml` — keeps the high-level overview
and calls focused reusable workflows for linting, tests, builds,
security scanning, and dashboard E2E checks. The jobs
still run every PR touches on stock `ubuntu-latest` runners, with
stock `actions/cache`, stock `docker/buildx` + QEMU for arm64, and
stock `services:` containers. It is slow because it does real work
across the whole monorepo, not because it has been deliberately
pessimized.
...
| `sdk-conformance` | shared 25-fixture corpus run against Go SDK and TS SDK | `make sdk-conformance` |
...
| `dashboard-e2e` | full compose stack up, Playwright-owned fixture setup, happy-path against the running dashboard on `:3000` | `make dashboard-e2e` |
...
2. **Cold buildx cache per branch.** `actions/cache` is keyed on the branch name with `main` as the universal restore-key. The first push on a new branch pays a cold cache.
3. **Compose-up E2E job.** `dashboard-e2e` does a `docker compose up -d --build` from cold. Sharing the build by pushing pre-built images between jobs is exactly the kind of thing slice 7b will demonstrate.
...
- Compose-up jobs that re-build everything → shared image hand-off via Depot's ephemeral registry.
