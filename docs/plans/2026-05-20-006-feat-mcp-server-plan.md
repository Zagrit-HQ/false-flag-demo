...
The planning notes calls this out under Fan-Out D (prerequisite: API contract skeleton — shipped in slice 3) and describes the same six tools.
...
- [ ] `docker-bake.hcl` default group includes `"mcp"`.
...
Owned paths: `infra/compose.yaml`, `docker-bake.hcl`, `tests/hurl/12-mcp-tools.hurl` (new), `scripts/mcp-smoke.sh` (new), `Makefile`, `cmd/falseflag-mcp/README.md`, `AGENTS.md`.
...
2. `docker-bake.hcl`: add `"mcp"` to `group "default" targets`. The `target "mcp"` block already exists, no other changes.
...
Run all of these at the end of slice 6.
...
- `docker-bake.hcl` (`mcp` joins default group)
...
Implementation summary skeleton:
...
- `docker-bake.hcl` — existing `mcp` target, default group definition
