...
validation ladder in `docs/METAPLAN.md` pass end to end:
...
`docs/METAPLAN.md` requires Slice 1 to land before any parallel fan-out
(`Configuration Strategies`, `Contracts/Operator/Clients`,
`User-Facing Surfaces`, `MCP/CI`) can start without contention on root files.
...
6. **Single Dockerfile per language** under `infra/`: `infra/Dockerfile.go`
   ...
   `infra/docker-bake.hcl` declares targets `api`, `proxy`, `operator`,
   `dashboard`.
...
8. **CONTRIBUTING.md** capturing layout rules so later parallel workers do
   ...
   `internal/gen/**`, owned paths follow `docs/METAPLAN.md`.
...
slice 7). It only requires `docker buildx bake --print -f infra/docker-bake.hcl`
...
bake-print:
	docker buildx bake --print -f infra/docker-bake.hcl
...
- Root files (`go.mod`, `js/package.json`, `Makefile`, `buf.*`,
  `sqlc.yaml`, `docker-bake.hcl`) are main-thread-owned; see
  `docs/METAPLAN.md`.
...
- [x] `docs/METAPLAN.md` Status Notes appended with the run commands and
      results.
...
- Compile Contract from `docs/METAPLAN.md` passes end-to-end.
- HTTP Contract minimum (`make smoke` against `/healthz`) passes.
- Subsequent slices can fan out (per `docs/METAPLAN.md`
  parallelization map) without touching root tooling files.
...
parallel fan-out per `docs/METAPLAN.md` Sequential Gate #2.
...
Run the validation ladder from `docs/METAPLAN.md`:
...
Update `docs/METAPLAN.md` Status Notes with the actual command results,
mark Checklist items "Implement slice 1: Foundation monorepo" and
"Verify slice 1 locally" complete only when every command above
succeeds.
...
- `docs/architecture/README.md` populated with a one-page diagram of
  ...
  slice.
- `docs/METAPLAN.md` Status Notes appended after validation passes.
...
- `docs/METAPLAN.md` — Operating mode, dependency map, validation
  ladder; specifies this slice as Sequential Gate #2.
...
- Next: `docs/METAPLAN.md` recommended sequence step 2 —
  Configuration Strategies plan (`ce-plan` prompt already drafted in
  METAPLAN).
