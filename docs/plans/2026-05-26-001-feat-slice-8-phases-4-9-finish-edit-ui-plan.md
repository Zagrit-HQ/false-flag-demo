...
- **Phase 9** — tick the slice-8 checkbox in `docs/METAPLAN.md`, write a status note, update `internal/config/README.md` + `js/packages/config-ts/README.md` to describe the new pipeline. (`infra/Dockerfile` and `.github/workflows/ci.yml` do **not** change — both new Go deps are pure Go, and CI auto-triggers are intentionally off ahead of slice 7b.)

PR grouping per the METAPLAN status note: Phases 4-6 ship as one PR (backend correctness + tests). Phases 7-8 ship together (view+edit is one user-visible delta). Phase 9 ticks the checklist when both PRs land. CI workflow auto-triggers stay off (slice 7b owns re-enabling them).
...
- Tick `- [x] Implement slice 8: Server-side TS compile + view/edit source UX`.
- Tick `- [x] Verify slice 8 locally`.
- Add a `2026-05-26: Slice 8 (Server-side TS compile + view/edit source UX) shipped on main` status note summarizing the two PRs, key files, and validation ladder results.
- Update the "Next recommended step is …" pointer to `Plan slice 9: Polish and demo script`.
...
Tasks:
- Update `docs/METAPLAN.md` checkbox + status note.
...
Success criteria:
- `git diff docs/METAPLAN.md` shows the two ticks + the new status note + the pointer update.
...
- [ ] `docs/METAPLAN.md` slice 8 checkboxes ticked + status note + "next step" pointer updated.
...
- `docs/METAPLAN.md` — checkbox + status note + pointer (per Phase 9).
