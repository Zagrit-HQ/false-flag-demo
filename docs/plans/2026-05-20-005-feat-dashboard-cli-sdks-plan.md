...
The planning notes's quality bar is "broad credible surface area, visible
...
### Worker tracks (per planning notes Fan-Out C)
...
| Main thread | `Makefile`, `js/package.json`, `js/turbo.json`, `js/pnpm-workspace.yaml`, `go.mod`, `go.sum`, `infra/compose.yaml`, `docker-bake.hcl`, `db/seed/**`, `cmd/falseflag-seed/**`, `docs/**`, generated artifacts | 1, 9 |
...
- No realtime SSE/streaming snapshot push. Slice 5 keeps it deferred.
...
- [ ] `docker-bake.hcl` already has a `proxy` target from slice 1.
...
each phase is independently reviewable. Keeping the convention preserves the slice-1
...
Rejected because the planning notes explicitly defers full OpenFeature
...
because the planning notes defers hardened auth and the slice 3 `X-Actor`
