...
This is the slice that finally wires up two items deferred since slice 1: `golangci-lint` end-to-end and the Playwright dashboard E2E.
...
**Generated code first.** The JS jobs (`typecheck-js`, `test-js`) need `pnpm -r generate` (orval against `api/openapi/openapi.yaml`) before they can run, because `@falseflag/dashboard` imports from `@falseflag/generated-client`. Each job runs its own `pnpm install && pnpm -r generate` — no artifact sharing between jobs.
...
| 2 | Multi-arch buildx on PR? | **Always multi-arch** | The repo ships multi-arch images per `docker-bake.hcl:24`. PR-amd64-only is a real optimization; skip it so slice 7b can show the QEMU savings. |
...
**Goal:** verify `docker-bake.hcl` builds headless under CI conditions and that the buildx local-cache mounts in the Dockerfiles cooperate with `actions/cache`.
...
**Files touched:** `docker-bake.hcl` (optional tweak), maybe `Makefile`.
...
**Goal:** all of `test-go`, `typecheck-js`, `test-js`, `conformance` jobs added.
...
- `timing-summary` job: `needs: [generate-check, lint-go, lint-js, typecheck-js, test-go, test-js, contract-test, conformance, build-images, smoke, dashboard-e2e, kind-smoke]`, `if: always()`.
...
- Record the verification command list with green checkmarks.
- Record known gaps deferred (image push, branch protection, path filters — all explicitly out of scope).
- Record baseline timing numbers (the headline figures slice 7b will improve).

**Done when:** the slice 7a PR has a fully green check column with the timing summary visible in the Actions UI.
...
- [ ] `make lint-go` (golangci-lint) runs and exits 0 in the `lint-go` job.
- [ ] `make dashboard-e2e` runs and exits 0 in the `dashboard-e2e` job.
...
- **Bake config:** `docker-bake.hcl` — `default` group, multi-arch platforms, REGISTRY var.
