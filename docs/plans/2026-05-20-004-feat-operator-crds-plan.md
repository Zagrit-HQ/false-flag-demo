...
The metaplan's product narrative claims feature flags can be authored
...
- **API group `falseflag.dev`.** Matches the existing CRD already on
  disk (`deploy/crds/falseflag.dev_projects.yaml`). The metaplan
...
- [ ] `infra/docker-bake.hcl` gains a `falseflag-operator` target.
...
- `infra/docker-bake.hcl` (modified — add `falseflag-operator`
...
METAPLAN status notes updated. Plan flipped to `status: completed`.

**Owns:**
- `scripts/kind-smoke.sh` (new — executable, kind up/down + apply +
  curl assertions)
- `Makefile` (modified — `kind-smoke` target + `make help` entry)
- `docs/METAPLAN.md` (modified — tick slice-4 checklist + status
  note mirroring slice 3's format)
...
- METAPLAN slice-4 checklist all checked
...
that satisfies the metaplan's Kubebuilder requirement.
...
- `docs/METAPLAN.md` status note (close-out).

## Sources & References

### Internal References

- `docs/METAPLAN.md` §"Recommended ce-plan Sequence > 4. Kubernetes
  Operator and CRDs Plan" (canonical brief)
- `docs/METAPLAN.md` §"Quality Bar" (demo-quality contract)
...
- `infra/docker-bake.hcl` (existing targets)
