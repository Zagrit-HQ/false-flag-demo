...
The compose stack now includes the `mcp` service. To bring everything
up, seed demo data, and run the smoke:

```bash
make up        # builds + starts api, proxy, operator, mcp, dashboard
make seed      # populates 3 projects + 7 flags
make mcp-smoke # initialize → tools/list → tools/call list_projects
```
